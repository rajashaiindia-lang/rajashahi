...
function DateRangeCell({ start, end }: { start?: string; end?: string }) {
  return (
    <div className="relative bg-[#fffdf6] rounded-[6px] border border-black/40 py-2 flex flex-col items-center justify-center text-[9px] text-yellow-800/90">
...
export default function DayResultsTable() {
  const { data } = useSWR<{ items: Item[] }>('/api/result/history?weeks=52&side=day', fetcher);
